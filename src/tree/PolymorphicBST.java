...
public class PolymorphicBST<K extends Comparable<K>, V> {
	...
	public Set<K> keySet() {
		...
		root.addKeysToCollection(hashSet);
		return hashSet;
	}
	 ...
	 * efficient avoiding unnecessary traversals of the tree. Also you MAY not add
	 * an auxiliary method to support the implementation of this method.
	 * 
	 * @return PolymorphicBST consisting of subset of SearchTreeMap
	 */
	public PolymorphicBST<K, V> subMap(K fromKey, K toKey) {
		Tree<K, V> subTree = root.subTree(fromKey, toKey);
		PolymorphicBST<K, V> polySubTree = new PolymorphicBST<K, V>();
		ArrayList<K> keySet = new ArrayList<K>();
		subTree.addKeysToCollection(keySet);
		for (K key : keySet) {
			polySubTree.put(key, subTree.search(key));
		}
		return polySubTree;
	}
	...
	/**
	 * Performs an in-order traversal applying the task to eat tree key,value pair.
	 * 
	 * @param p
	 */
